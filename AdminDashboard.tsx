...
import { LanguageSwitcher, ClientsSection, TestimonialsSection, ContactForm, FAQSection, ProcessVisualizer, LucideIcon, Highlight, UserProfile, TrackOrder } from "./frontend/components";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const scrollInterval = setInterval(handleScroll, 300);
    handleScroll();
  ...
  return (
    <ThemeContext.Provider value={{ isDark, toggleTheme }}>
      <LanguageContext.Provider value={{ lang, setLang, t }}>
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
            <div className="flex justify-between items-center h-20 font-sans">
              ...
              <div className="hidden md:flex items-center gap-6 lg:gap-8 font-medium text-sm">
                ...
                <a href="#track-order" className="text-neutral-600 dark:text-neutral-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors uppercase tracking-wider text-sm font-black">{t.nav.trackOrder}</a>
            ...
            {isMenuOpen && (
              ...
              >
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <a href="#track-order" onClick={() => setIsMenuOpen(false)} className="px-4 py-3 rounded-xl hover:bg-neutral-50 dark:hover:bg-neutral-800 font-bold text-neutral-600 dark:text-neutral-300 transition-colors">{t.nav.trackOrder}</a>
                </div>
        ...
        {/* Track Order Section */}
        <section className="py-24 bg-neutral-100/40 dark:bg-neutral-900/40 border-t border-neutral-200/50 dark:border-neutral-800/50 transition-colors">
          <TrackOrder />
        </section>
      ...
      {/* Floating overlays for Front-End view */}
      {/* Back to Top Button */}
      <button
        onClick={scrollToTop}
        id="back-to-top"
        className="fixed bottom-24 right-8 z-[999999] bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-full shadow-2xl border border-blue-500/30 hover:scale-110 active:scale-95 cursor-pointer transition-all duration-300 transform flex items-center justify-center opacity-100 translate-y-0 scale-100 pointer-events-auto"
      ...
      <a
        href="https://wa.me/8562056618853"
        target="_blank"
        rel="noopener noreferrer"
        id="whatsapp-floating"
        className="fixed bottom-6 right-8 z-[999999] bg-[#25D366] hover:bg-[#20BA56] text-white p-4 rounded-full shadow-2xl border border-green-400/30 hover:scale-110 active:scale-95 cursor-pointer transition-all duration-300 flex items-center justify-center group opacity-100 translate-y-0 scale-100 pointer-events-auto"
        aria-label="Chat on WhatsApp"
      >
        <span className="absolute right-14 bg-neutral-900/95 text-white text-xs font-black py-2 px-3 rounded-xl opacity-0 translate-x-2 pointer-events-none group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 whitespace-nowrap shadow-xl border border-neutral-800">
          {lang === "lao" ? "ຕິດຕໍ່ທາງ WhatsApp" : lang === "thai" ? "ແຊັດຜ່ານ WhatsApp" : "Chat on WhatsApp"}
        </span>
        <span className="absolute w-3 h-3 bg-green-400 rounded-full animate-ping top-0 right-0 border-2 border-green-500" />
